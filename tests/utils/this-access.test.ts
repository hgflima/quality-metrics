...
describe('extractThisAccesses', () => {
  ...
  it('captures a single `this.foo` access', () => {
    expect(extractThisAccesses(method(exprStmt(thisDot('foo'))))).toEqual(
      new Set(['foo']),
    );
  });
  ...
  it('captures multiple distinct properties', () => {
    expect(
      extractThisAccesses(
        method(exprStmt(thisDot('db')), exprStmt(thisDot('mailer'))),
      ),
  ...
  it('captures only the first hop in a chain like `this.foo.bar`', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(chain)))).toEqual(
      new Set(['foo']),
    );
  });
  ...
  it('captures `this.foo()` (method call)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(call)))).toEqual(
      new Set(['save']),
    );
  });
  ...
  it('captures `this.foo = value` (assignment LHS)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(assign)))).toEqual(
      new Set(['count']),
    );
  });
  ...
  it('captures `this.count++` (update expression)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(update)))).toEqual(
      new Set(['count']),
    );
  });
  ...
  it('does NOT capture bare `this` without member access', () => {
    expect(
      extractThisAccesses(
        method({ type: 'ReturnStatement', argument: thisExpr() }),
      ),
  ...
  it('captures `this[\'foo\']` (computed string-literal key)', () => {
    expect(
      extractThisAccesses(method(exprStmt(thisComputed(lit('foo'))))),
    ).toEqual(new Set(['foo']));
  });
  ...
  it('captures Babel-style `StringLiteral` computed key', () => {
    expect(
      extractThisAccesses(
        method(
          exprStmt(thisComputed({ type: 'StringLiteral', value: 'mailer' })),
        ),
      ),
  ...
  it('does NOT capture `this[x]` (dynamic computed key)', () => {
    expect(
      extractThisAccesses(method(exprStmt(thisComputed(id('x'))))),
    ).toEqual(new Set<string>());
  });

  it('does NOT capture `this[42]` (numeric computed key)', () => {
    expect(
      extractThisAccesses(method(exprStmt(thisComputed(lit(42))))),
    ).toEqual(new Set<string>());
  });
  ...
  it('treats `#foo` and `foo` as distinct properties', () => {
    expect(
      extractThisAccesses(
        method(exprStmt(thisDot('count')), exprStmt(thisPrivate('count'))),
      ),
  ...
  it('TRAVERSES into ArrowFunctionExpression — `this` is shared', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(arrow)))).toEqual(
      new Set(['shared']),
    );
  });
  ...
  it('TRAVERSES nested arrows (block body)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(outer)))).toEqual(
      new Set(['outer', 'inner']),
    );
  });
  ...
  it('does NOT traverse a nested FunctionDeclaration', () => {
    ...
    expect(
      extractThisAccesses(
        method(exprStmt(thisDot('outer')), nested),
      ),
  ...
  it('does NOT traverse a nested FunctionExpression', () => {
    ...
    expect(
      extractThisAccesses(method(exprStmt(thisDot('outer')), exprStmt(nested))),
    ).toEqual(new Set(['outer']));
  });
  ...
  it('does NOT traverse a nested class declaration (its methods bind their own `this`)', () => {
    ...
    expect(
      extractThisAccesses(method(exprStmt(thisDot('outer')), nestedClass)),
    ).toEqual(new Set(['outer']));
  });
  ...
  it('handles low-cohesion fixture shape (one prop per method)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(callOnDb)))).toEqual(
      new Set(['db']),
    );
  });
  ...
  it('captures `this.foo` reached via optional chaining (`this.foo?.bar`)', () => {
    ...
    expect(extractThisAccesses(method(exprStmt(optional)))).toEqual(
      new Set(['foo']),
    );
  });
});
