...
- `.git/` directory
- `src/`, `specs/`, `docs/`, `.planning/` directories
- Home directory (`~`, `$HOME`)
...
0a. Study docs/mvp/* (use up to 500 parallel Sonnet subagents)
...
3. Implement
   ...
   - Reference docs/mvp/* for requirements
   - Write clean, maintainable code
   - Match existing code style and conventions
   - Add tests if they don't exist for new functionality

4. Validate
   - Run tests: `npm test` (or `pnpm test` / `yarn test` depending on lockfile)
   - Run type check: `npx tsc --noEmit` (or the project's configured type-check script)
