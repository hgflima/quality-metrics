...
check_all_tasks_complete() {
  ...
  local incomplete
  incomplete=$(grep -c '^\s*- \[ \]' "$PLAN_FILE" 2>/dev/null | head -1)
  incomplete=${incomplete:-0}

  if [ "$incomplete" -eq 0 ]; then
    # Double-check there are actually completed tasks
    local completed
    completed=$(grep -c '^\s*- \[x\]' "$PLAN_FILE" 2>/dev/null | head -1)
...
print_iteration_summary() {
  ...
  if [ -n "$commit_msg" ]; then
    ...
    files_new=$(echo "$new_files" | grep -c . 2>/dev/null | head -1)
    files_modified=$(echo "$modified_files" | grep -c . 2>/dev/null | head -1)
  ...
  completed=$(grep -c '^\s*- \[x\]' "$PLAN_FILE" 2>/dev/null | head -1)
  total_tasks=$(grep -c '^\s*- \[' "$PLAN_FILE" 2>/dev/null | head -1)
  completed=${completed:-0}
...
generate_report() {
  ...
  local completed=$(grep -c '^\s*- \[x\]' "$PLAN_FILE" 2>/dev/null | head -1)
  local skipped=$(grep -c '^\s*- \[S\]' "$PLAN_FILE" 2>/dev/null | head -1)
  local remaining=$(grep -c '^\s*- \[ \]' "$PLAN_FILE" 2>/dev/null | head -1)
  completed=${completed:-0}
  skipped=${skipped:-0}
...
# Don't fail the loop if backup setup fails (set -e would kill us otherwise)
